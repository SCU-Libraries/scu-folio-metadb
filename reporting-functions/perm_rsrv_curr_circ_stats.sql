--metadb:function perm_rsrv_curr_circ_stats
DROP FUNCTION IF EXISTS perm_rsrv_curr_circ_stats;

-- Output circ counts (checkouts + renewals) for all items currently attached to 
...
CREATE OR REPLACE FUNCTION perm_rsrv_curr_circ_stats (
)
...
WITH
	trms AS (
		...
		WHERE
			lower(trim(trm.jsonb->>'name')) = 'permanent'
	),
	cls AS (
		SELECT
			DISTINCT ON (cl.id)
		...
		FROM
			folio_courses.coursereserves_courselistings cl
		LEFT JOIN
		...
		LEFT JOIN LATERAL (
			...
			FROM
				folio_courses.coursereserves_courselistings clpi
			WHERE
				clpi.__id = cl.__id
	...
	crss AS (
		SELECT
			...
			cls.cls_end
		FROM
			folio_courses.coursereserves_courses crs
		LEFT JOIN
	...
	crr_items AS (
		SELECT DISTINCT ON (crr.id, crr.courselistingid)
			...
			END AS call_number
		FROM
			folio_courses.coursereserves_reserves crr
		LEFT JOIN
	...
	ORDER BY
		course_name, course_number, crr_items.crr_end, item_title, call_number
